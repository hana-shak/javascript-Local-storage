...
function addUser () { 
    $new_arr=array('user'=> $_POST['username'] ,'password'=> $_POST ['password'], 'role'=>"user", 'phone'=> $_POST ['phone']);
    array_push($_SESSION ['mixed-sess'],$new_arr);
    //echo "<pre>";
    //print_r($_SESSION ['mixed-sess']);
} 

$nameErr =  $name = $pass = $passErr = $passmaErr = $phoneErr = $phone = $passma =" ";
...
if (isset($_POST['register']) && isset($_POST["password"]) && isset($_POST["register"]) &&
  ...
  /*welcome-admin */
      foreach ($_SESSION ['mixed-sess'] as $key => $value){ 
        ...
        if (!(empty($_POST["username"]) && empty($_POST["password"]) && empty($_POST["pass"]) && empty($_POST["phone"]))) {
              addUser();}
print_r($_SESSION ['mixed-sess']);
?>
...
<body>
  ...
  <div class="container">
        <div class="col-sm-6">
           ...
           <form action="soso-reg.php" method="POST">
